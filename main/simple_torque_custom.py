...
def main(fish_turn_speed, shark_turn_speed, visuals_on, seed):
    ...
    # Fiskens koordinater
    x = [-30, -20, -10, 0, 10, 20, 30, -30, -20, -10, 0, 10, 20, 30, -30, -20, -10, 0, 10, 20, 30]
    y = [0, 0, 0, 0, 0, 0, 0, 10, 10, 10, 10, 10, 10, 10, 20, 20, 20, 20, 20, 20, 20]
    fish_coords = np.column_stack((x, y))
    fish_orientations = np.zeros(21) - np.pi/2  # orientations
    fish_desired_orientations = fish_orientations.copy()  # Array med alla fiskars önskade riktning

    # Startkoordinater hajar
    shark_coords = np.column_stack((0, -80))  # Array med alla hajars x- och y-koord
    shark_orientations = [np.pi/2]  # Array med alla hajars riktning
