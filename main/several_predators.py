...
simulation_iterations = 400  # Antalet iterationer simulationen kör
...
def murder_fish_coords(fish_coords, dead_fish_index):  # Tar bort fisk-coord som blivit uppäten
    new_fish_coords = np.delete(fish_coords, dead_fish_index, 0)
    return new_fish_coords


def murder_fish_orientations(fish_orientations, dead_fish_index):  # Tar bort fisk-vinkel som blivit uppäten
    new_fish_orientations = np.delete(fish_orientations, dead_fish_index)
    return new_fish_orientations

def predict_position(fish_coord, fish_orientation, distance_to_fish):   # Förutspår positionen av en fisk beroende på avstånd till fisk
...
def main():
    ...
    for t in range(simulation_iterations):
        ...
        for j in range(len(fish_coords)):
            ...
            if shark_fish_distance_matrix[
                closest_shark, j] < fish_interaction_radius:  # Om hajen är nära fisken, undvik hajen
        ...
        if len(fish_coords) > 1:  # <- den if-satsen är för att stoppa crash vid få fiskar
            for j in range(shark_count):
                ...
                if shark_closest_fish_distances[j] < murder_radius:  # Allt som händer då en fisk blir äten
                    last_index = len(
                        fish_coords) - 1  # Sista index som kommer försvinna efter den mördade fisken tas bort

                    if visuals_on:
                        canvas.delete(fish_canvas_graphics[last_index])  # Ta bort sista fisk-cirkeln i array

                    fish_coords = murder_fish_coords(fish_coords, closest_fish[j])  # Tar bort index i koordinaterna
                    fish_orientations = murder_fish_orientations(fish_orientations, closest_fish[j])  # Tar bort index i orientations
    ...
    # plt.show()
    np.save('fish_eaten.npy', fish_eaten)
