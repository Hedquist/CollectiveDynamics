...
obstacle_count = np.linspace(start_obst_count, end_obst_count, 6, dtype=int)  # Ger start till end
...
obstacle_count = [9, 16, 25, 36, 49, 64]
...
markers, caps, bars = ax.errorbar(obstacle_count, fish_eaten_mean, yerr=fish_eaten_std, fmt='bo')
plt.xlabel('Antal hinder ')
plt.ylabel('Medelvärde av antal fångade bytesdjur')
#plt.title('Medelvärde av antalet fångade bytesdjur som funktion av antalet hinder')
