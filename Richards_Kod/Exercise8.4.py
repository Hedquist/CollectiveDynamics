...
sharky=[50]
sharkv=1.8
sharkphi=np.random.rand(len(sharkx))*2*np.pi
shark = []
...
for i in range(len(sharkx)):
    shark.append(canvas.create_oval((sharkx[i] - R + l) * res / l / 2,
                                        (sharky[i] - R + l) * res / l / 2,
                                        (sharkx[i] + R + l) * res / l / 2,
                                        (sharky[i] + R + l) * res / l / 2,
...
for t in range(4000):
    ...
    sharkx = (sharkx + sharkv * np.cos(sharkphi)*dt + l) % (2 * l) - l
    sharky = (sharky + sharkv * np.sin(sharkphi)*dt + l) % (2 * l) - l
    sharkdists = np.minimum(np.sqrt(
        ((r[:, 0]) % (2 * l) - sharkx % (2 * l)) ** 2 + ((r[:, 1]) % (2 * l) - sharky % (2 * l)) ** 2),
        np.sqrt((r[:, 0] - sharkx) ** 2 + (r[:, 1] - sharky) ** 2))
    #print(sharkdists)
    #print(sharkdists.shape)

    closest_fish = np.where(sharkdists == np.amin(sharkdists))[0]

    if np.sqrt(((r[closest_fish, 0]) % (2 * l) - sharkx % (2 * l)) ** 2 + ((r[closest_fish, 1]) % (2 * l) - sharky % (2 * l)) ** 2)<np.sqrt((r[closest_fish, 0] - sharkx) ** 2 + (r[closest_fish, 1] - sharky) ** 2):
        sharkphi = np.arctan2((r[closest_fish, 1]) % (2 * l) - sharky % (2 * l), (r[closest_fish, 0]) % (2 * l) - sharkx % (2 * l))
    else:
        sharkphi = np.arctan2((r[closest_fish, 1] - sharky), (r[closest_fish, 0] - sharkx))
    ...
    for j in range(len(shark)):
        canvas.coords(shark[j],
                    (sharkx[j] - R + l) * res / l / 2,
                    (sharky[j] - R + l) * res / l / 2,
                    (sharkx[j] + R + l) * res / l / 2,
                    (sharky[j] + R + l) * res / l / 2,)
    for j in range(N):
        ...
        interact = dist < Rf
        if sharkdists[j] < Rf:
            if np.sqrt(((r[j, 0]) % (2 * l) - sharkx % (2 * l)) ** 2 + (
                    (r[j, 1]) % (2 * l) - sharky % (2 * l)) ** 2) < np.sqrt(
                    (r[j, 0] - sharkx) ** 2 + (r[j, 1] - sharky) ** 2):
                phi[j] = np.arctan2((r[j, 1]) % (2 * l) - sharky % (2 * l),
                                      (r[j, 0]) % (2 * l) - sharkx % (2 * l))
            else:
                phi[j] = np.arctan2((r[j, 1] - sharky), (r[j, 0] - sharkx))
        else:
