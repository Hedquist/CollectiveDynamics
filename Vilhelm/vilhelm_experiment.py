...
set_phi = phi.copy()  # initial desired orientations
...
def mass_turn(desired_phi, current_phi, mass):
    relative_phi = current_phi - desired_phi    # makes calculation easier
    if relative_phi == 0:
...
def update_orientation(phi, set_phi, mass): # uses mass_turn to update phi array according to set_phi
    for n in range(len(phi)):
        phi[n] = mass_turn(set_phi[n], phi[n], mass)
    return phi
...
for t in range(4000):
    phi = update_orientation(phi, set_phi, m)   # update orientations before updating positions
    r = update_position(r, v, phi, dt)
    phi_shark = update_orientation(phi_shark, set_phi_shark, m_shark)   # same for the shark
    ...
    if np.sqrt(((r[closest_fish, 0]) % (2 * l) - r_shark[0, 0] % (2 * l)) ** 2 + (
            ...
            (r[closest_fish, 0] - r_shark[0, 0]) ** 2 + (r[closest_fish, 1] - r_shark[0, 1]) ** 2):
        set_phi_shark = np.arctan2((r[closest_fish, 1]) % (2 * l) - r_shark[0, 1] % (2 * l),    # update set_phi instead
                               (r[closest_fish, 0]) % (2 * l) - r_shark[0, 0] % (2 * l))        # of phi
    else:
        set_phi_shark = np.arctan2((r[closest_fish, 1] - r_shark[0, 1]), (r[closest_fish, 0] - r_shark[0, 0]))
    ...
    for j in range(N):
        ...
        if sharkdists[j] < Rf:
            if np.sqrt(((r[j, 0]) % (2 * l) - r_shark[0, 0] % (2 * l)) ** 2 + (
                ...
                set_phi[j] = np.arctan2((r[j, 1]) % (2 * l) - r_shark[0, 1] % (2 * l),  # update set_phi instead of phi
                                    (r[j, 0]) % (2 * l) - r_shark[0, 0] % (2 * l))
            else:
                set_phi[j] = np.arctan2((r[j, 1] - r_shark[0, 1]), (r[j, 0] - r_shark[0, 0]))
        else:
            set_phi[j] = np.angle(np.sum(np.exp(phi[interact] * 1j))) + eta * np.random.uniform(-1 / 2, 1 / 2)
