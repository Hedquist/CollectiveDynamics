...
fish_arrow_length = fish_graphic_radius # Pillängd
fish_ray_radius = fish_interaction_radius/2 +2 # Strållängd
...
# Fiskens koordinater
if visual_debug:
    x = np.array(np.ones(fish_count) * 0)
    y = np.array(np.ones(fish_count) * 80)
    fish_coords = np.column_stack((x, y))
    fish_orientations = np.ones(fish_count) * np.pi/2  # orientations
else:
...
with open('Environments/Environment', 'r') as filestream:
...
def calculate_distance_circ_to_rect(circ_coord, circ_radius, rect_corners_coords, many_rectangles, calculate_distance):
    ...
    # Tar fram de närmsta punkten
    NearestX = np.maximum(X1, np.minimum(Xc, X2))
    NearestY = np.maximum(Y1, np.minimum(Yc, Y2))

    # Konventionella vståndet från närmsta punkten på rektangeln till cirkelns centrum
    Dx_convent = Xc - NearestX
    Dy_convent = Yc - NearestY
    ...
    dist_convent = np.absolute(np.sqrt(Dx_convent ** 2 + Dy_convent ** 2)-R)
    dist_modulo = np.absolute(np.sqrt(Dx_modulo ** 2 + Dy_modulo ** 2)-R)
    dist = np.minimum(dist_convent, dist_modulo)
...
for t in range(simulation_iterations):
    ...
    for j in range(len(fish_coords)):  # Updating animation coordinates fisk
        ...
        if detect_obst_in_radius_info[0]:
            ...
            rect_obst_detect_ind = obst_type_in_radius[0] # De hindren som detekterats
            circ_obst_detect_ind = obst_type_in_radius[1]
