...
for t in range(simulation_iterations):
    ...
    for j in range(len(fish_coords)):  # Updating animation coordinates fisk
        ...
        if True in detect_circ_obst: # Om detekterar hinder
            ...
            # positive_angle_obstacle = 2*np.pi + circ_obst_angle if circ_obst_angle < 0 else circ_obst_angle
            # positive_angle_fish = 2*np.pi + fish_orientations[j] if fish_orientations[j] < 0 else fish_orientations
            direction = np.sign(center_angle+fish_orientations[j])
            #print(np.rad2deg(circ_obst_angle))
            #print(np.rad2deg(fish_orientations[j]))
            force = 1/circ_obst_dist
            circ_obst_avoid_angle = force*direction*np.pi/2
