...
for i in range(len(rect_obst_coords)):
    rect_obst_corner_coords.append(calculate_rectangle_corner_coordinates(rect_obst_coords[i], obstacle_rectangular_width, obstacle_rectangular_height))
...
def draw_circular_obstacles():
    ...
    for j in range(circ_obst_coords.shape[0]):
        circ_obst_canvas_graphics.append(canvas.create_oval((x_circ_obst[j] - obstacle_radius + canvas_length) * res / canvas_length / 2,
...
def draw_rectangular_obstacles():
    for j in range(rect_obst_coords.shape[0]):
        rect_obst_canvas_graphics.append(canvas.create_rectangle((x_rect_obst[j] + obstacle_rectangular_width + canvas_length) * res / canvas_length / 2,
...
def wall_detect(rays_coords):
    ...
    return((True in rays_outside_wall,rays_outside_wall)) # Returnera sant eller falskt plus en lista vilka ray är utanför
...
def rect_obst_detect(rays_coords, rectangular_obstacle_coords, rectangular_obstacle_corners):
    ...
    for rectangle in range(len(rectangular_obstacle_coords)):
        for ray in range(len(rays_coords)):
            rays_inside_rectangular_obstacle[rectangle].append(is_point_outside_rectangle(rectangular_obstacle_corners[rectangle], rays_coords[ray],False))
...
for t in range(simulation_iterations):
    ...
    for j in range(len(fish_coords)): # Updating animation coordinates fisk
        ...
        if(wall_detect(rays_coords[j])[0]):
            wall_avoid_angle = avoid_wall(fish_coords[j], rays_coords[j], rays_angle_relative_velocity[j], fish_orientations[j], wall_detect(rays_coords[j])[1])

        if(circ_obst_detect(rays_coords[j], circ_obst_coords)):
            circ_index = circ_obst_closest_index(fish_coords[j], rays_coords[j], circ_obst_coords) # Index av närmaste cirkulära hinder
            circ_obst_rays = circ_obst_boolean_rays(rays_coords[j], circ_obst_coords)[circ_index] # Lista med vilka rays är upptagna
            circ_avoid_angle = circ_obst_avoid(fish_coords[j], rays_angle_relative_velocity[j], fish_orientations[j], circ_index, circ_obst_rays, rays_coords[j]) # Beräkna vinkeln
        ...
        if(rect_obst_detect(rays_coords[j], rect_obst_coords, rect_obst_corner_coords)):
            ...
            rect_obst_rays = rect_obst_boolean_rays(rays_coords[j], rect_obst_coords, rect_obst_corner_coords)[rect_index]
            rect_avoid_angle = rect_obst_avoid(fish_coords[j], rays_angle_relative_velocity[j], fish_orientations[j], rect_index, rect_obst_rays)
        ...
        fish_orientations[j] = np.angle(np.sum(np.exp(fish_orientations[fish_in_interaction_radius] * 1j))) + fish_noise * np.random.uniform(-1 / 2, 1 / 2) + wall_avoid_angle + circ_avoid_angle + rect_avoid_angle
